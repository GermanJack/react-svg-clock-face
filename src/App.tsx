...
const AnalogClock = (props: IProps) => {
  ...
  const settime = useCallback(
    (date: Date) => {
      if (clock.Mode === 1 || clock.Mode === 2) {
        ...
        setHandsToTime(clock.ID, `${h}:${m}:${s}`, clock.CCW, clock.Twentyfour)
      }
    },
  ...
  useEffect(() => {
    if (clock.Mode === 2) {
      drawHand(
        'MinuteHand' + clock.ID,
        clock.MinuteHandLength,
        clock.MinuteHandWidth,
        clock.MinuteHandPointy,
        clock.MinuteHandColor,
        SVGRef.current!,
        clock.MinuteHandRadius,
        clock.DrawHands,
      )
      drawHand(
        'HourHand' + clock.ID,
        clock.HourHandLength,
        clock.HourHandWidth,
        clock.HourHandPointy,
        clock.HourHandColor,
        SVGRef.current!,
        clock.HourHandRadius,
        clock.DrawHands,
      )
      drawHand(
        'SecondHand' + clock.ID,
        clock.SecondHandLength,
        clock.SecondHandWidth,
        clock.SecondHandPointy,
        clock.SecondHandColor,
        SVGRef.current!,
        clock.SecondHandRadius,
        clock.DrawHands,
      )
  ...
  useEffect(() => {
    if (clock.Mode === 3) {
      ...
      setHandsToTime(clock.ID, vt, clock.CCW, clock.Twentyfour)
    }
  }, [clock])
